...
namespace PacodelaCruz.DurableFunctions.Approval
{
    public static class MoveBlob
    {
        ...
        public static void Run([ActivityTrigger] ApprovalResponseMetadata responseMetadata, 
                                TraceWriter log)
        {
            log.Info($"Moving Blob {responseMetadata.ReferenceUrl} to {responseMetadata.Status}");
            try
            {
                ...
                log.Info($"Blob '{responseMetadata.ReferenceUrl}' moved to container '{responseMetadata.Status}'");
            }
            catch (Exception ex)
            {
